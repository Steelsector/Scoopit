...
function shouldRunOnPage(urlString) { try {
...
async function collectLazyLoadedRows(doc, maxRows, delayMs) {
    ...
    while (rounds < maxRounds && seen.size < maxRows && stagnantRounds < 10) {
        ...
        const nextTop = Math.min((scrollEl.scrollTop || 0) + stepPx, Math.max(0, (scrollEl.scrollHeight || 0) - viewportHeight));
        if (isDocScroller) {
...
function createRunState() {
    return {
        ...
        markSeen(key, usePersistent) { this.seenInRun.add(key); if (usePersistent)
...
async function findBestFolderButton(modal, doc, setupName) {
    ...
    if (yearSeason && personalChildren.length > 0) {
        ...
        const subfolder = personalChildren.find((item) => (item.text.includes(year) && item.text.replace(/\s+/g, "").includes(season)) ||
...
function closeAnyKnownModals(doc) {
    ...
    const globalCloseButtons = Array.from(doc.querySelectorAll("button.btn-close, .modal-footer button, app-setup-modal button, app-file-operation-modal button"));
        ...
        async importSetup(row, setupName) {
            ...
            for (let i = 0; i < 30; i += 1) {
                ...
                if (setupModal && !clickedAddToMySetups) {
                    const addBtn = Array.from(setupModal.querySelectorAll("button")).find((btn) => normalizeText(btn.textContent).includes("add to my setups"));
                    if (addBtn) {
                        addBtn.click();
                ...
                if (addFileModal) {
                    ...
                    folderButton.click();
                    await sleep(100);
                    const saveButton = Array.from(addFileModal.querySelectorAll(".modal-footer button")).find((btn) => normalizeText(btn.textContent) === "save");
                    if (!saveButton) {
                        closeAnyKnownModals(doc);
...
async function runImport() {
    ...
    try {
        ...
        function isScrollable(el) {
            if (!el)
                return false;
            if (el.closest(".modal, .modal-dialog, .modal-content, ngb-modal-window"))
                return false;
            const style = window.getComputedStyle(el);
            const overflowY = style.overflowY;
        ...
        while (loops < maxLoops && processed.size < maxRows && stagnantRounds < 12) {
            ...
            if (!progressedThisLoop) {
                ...
                const isDocScroller = scrollEl === document.scrollingElement || scrollEl === document.documentElement || scrollEl === document.body;
                const viewportHeight = isDocScroller ? (window.innerHeight || 0) : (scrollEl.clientHeight || 0);
                const nextTop = Math.min((scrollEl.scrollTop || 0) + step, Math.max(0, (scrollEl.scrollHeight || 0) - viewportHeight));
